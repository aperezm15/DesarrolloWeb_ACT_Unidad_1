    ...
    public static function loadClass(string $className): void
{
    ...
    $relativeFilePath = self::$classMap[$className];

    $filePath = $baseDir . str_replace(['/', '\\'], DIRECTORY_SEPARATOR, $relativeFilePath);

    if (!file_exists($filePath)) {
        // Si falla, lanzamos un mensaje que nos diga la ruta EXACTA que falló
        throw new RuntimeException(
            sprintf('ERROR CRÍTICO: El archivo no existe en la ruta: %s', $filePath)
        );
    }
